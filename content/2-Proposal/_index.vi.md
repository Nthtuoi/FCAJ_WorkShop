...
Trong workshop này, nhóm sẽ trình bày toàn bộ quá trình xây dựng hệ thống từ thiết kế kiến trúc, triển khai các dịch vụ AWS, viết code Lambda, tích hợp frontend React, kiểm thử pipeline và deploy website lên CloudFront. AWS Audio To Text – Hệ thống chuyển đổi audio thành văn bản trên AWS.

### 2. Mục tiêu

Mục tiêu chính của dự án là xây dựng một hệ thống chuyển đổi audio thành văn bản hoạt động tự động trên nền tảng AWS.
...
Có thể xem chi phí trên [AWS Pricing Calculator](https://calculator.aws/#/estimate?id=cf2cfdc1015110291d128e4a975ec054236c00df)  
...
- Nếu trigger chưa hoạt động, có thể test Lambda bằng event mẫu. Nếu frontend lỗi sau khi deploy, nhóm sẽ quay lại chạy local để kiểm tra API trước, sau đó build và deploy lại lên S3/CloudFront.
